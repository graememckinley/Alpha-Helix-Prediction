...
def check_file_format(path):
    ...
    with open(path) as fh:
        while True:
            ...
            if not matched1:
                raise UserWarning(
                    'Format of the description line may be wrong for file ' + path + "\nCorrect format example: '>d1dlwa | 116'")
            if not matched2:
                raise UserWarning(
                    'Format of the sequence line may be wrong for file ' + path + "\nCorrect format example: 'SLFEQLGGQAAVQA'")
            if not matched3:
                raise UserWarning(
                    'Format of the secondary structure line may be wrong for file ' + path + "\nCorrect format example: '--H-HH-H--H-H--'")
...
def compute_prediction_accuracy(preds, correct, output):
    ...
    for name in preds:
        ...
        if len(p) != len(c):
            raise UserWarning(
                'Different lengths between secondary structure predictions and secondary structure labels')
        total = total + len(p)
...
def main():
    ...
    parser.add_argument("-p", '--predictions',
                        help='Path to the file containing the secondary structure prediction to evaluate.')
    ...
    # Override the prediction files
    preds = read_fasta("../output_file/outfile.txt")
    correct = read_fasta("../training_data/labels.txt")
